...
class BaseChapter(arcade.View):
    """Общая инфраструктура: камеры, HUD (arcade.Text), звуки, переходы."""
    ...
    def __init__(self, chapter_num: int) -> None:
        ...
        self._grace: float = 0.5   # секунд после setup перед проверкой финиша
        self._grace_t: float = 0.0

        # arcade.Text объекты для HUD (быстрее draw_text)
        self._txt_top_left = arcade.Text("", 10, SCREEN_H - 28,
                                         (200, 200, 220), 13)
        self._txt_top_right = arcade.Text("", SCREEN_W - 10, SCREEN_H - 28,
                                          (200, 200, 220), 13,
                                          anchor_x="right")
        self._txt_center = arcade.Text("", SCREEN_W // 2, SCREEN_H - 28,
                                       (230, 210, 180), 13,
                                       anchor_x="center")
    ...
    def _can_finish(self) -> bool:
        """Возвращает True только когда прошёл grace-период после setup."""
